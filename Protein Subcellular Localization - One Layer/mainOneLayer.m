...
hiddenSizeL1 = 50;       % number of hidden units of layer 1
hiddenSizeL2 = 200;       % number of hidden units of layer 1
...
figure(1);
displayProteinFeatures(features1', 1, 1, 20, 0, 7, 20);
title('First-level features')
set(gcf,'name','window title string','numbertitle','off')

%----------------------------------------------
%% Step 4: Detect second-level protein features
%  This trains the second sparse autoencoder on the first autoencoder
%  featurse.
fprintf('\nPART 4: DETECT SECOND0LEVEL PROTEIN FEATURES\n');
fprintf('============================================\n');
fprintf('-- Randomly initializing theta of the second hidden layer...\n');
sparsityParam = 0.25;      
% Use a higher sparsity parameters for the second layer
% sae1OptTheta
...
fprintf('-- Detecting second-level features...\n');
...
% Visualize 2nd-level features
% Stacking layers of 1st-level features to form second level features
features2 = reshape(sae2OptTheta(1:hiddenSizeL2*hiddenSizeL1), hiddenSizeL2, hiddenSizeL1);

features2Visualize = zeros(hiddenSizeL2, inputSize);
for featureNum = 1:hiddenSizeL2
    features2Visualize(featureNum,:) = sum(bsxfun(@times,features1,(features2(featureNum,:))'));
end
% Visualizing 2nd-level features
figure(2);
displayProteinFeatures(features2Visualize', 1, 1, 20, 0, 7, 20);
title('Second-level features')
set(gcf,'name','Second-level Features','numbertitle','off')
...
for proteinNum = 1:numTest
    ...
    predictedLabels(proteinNum) = predictType(stackedAETheta, inputSize, hiddenSizeL2, ...
...
for proteinNum = 1:numTest
    protein = testProteins(:,proteinNum);
    predictedLabels(proteinNum) = predictType(stackedAEOptTheta, inputSize, hiddenSizeL2, ...
                          numClasses, netconfig, protein, windowSize);
end
acc = mean(testLabels(:) == predictedLabels(:));
